        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            if (show_results)
            {
                ...
                if (e == 1)
                {
                    ...
                    mergeVector.resize(10);
                    if (ImGui::BeginTable("table_sorting", 7, tableFlags, ImVec2(0.0f, TEXT_BASE_HEIGHT * 100), 0.0f))
                    {
                        // Declare columns
                        ...
                        ImGui::TableSetupColumn("City", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_Name);
                        ImGui::TableSetupColumn("Population", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_population);
                        ImGui::TableSetupColumn("Size", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_size);
                        ImGui::TableSetupColumn("Density", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_density);
                        ImGui::TableSetupColumn("Avg. Housing Cost", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_avgHousingCost);
                        ImGui::TableSetupColumn("Avg. Income", ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_avgIncome);
                        ImGui::TableSetupColumn("Score", ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthFixed, 0.0f, MyItemColumnID_weightScore);
                        ...
                        if (ImGuiTableSortSpecs* sorts_specs = ImGui::TableGetSortSpecs())
                            if (sorts_specs->SpecsDirty)
                            {
                                ...
                                if (mergeVector.size() > 1)
                                    qsort(&mergeVector[0], mergeVector.size(), sizeof(mergeVector[0]), MyItem::CompareWithSortSpecs);
                                MyItem::s_current_sort_specs = NULL;
                                sorts_specs->SpecsDirty = false;
                            }
                        ImGuiListClipper clipper;
                        clipper.Begin(mergeVector.size());
                        while (clipper.Step())
                                                                            //clipper.DisplayEnd for whole vector
                            for (int row_n = clipper.DisplayStart; row_n < clipper.DisplayEnd; row_n++)
                            {
                                // Display a data item
                                City* item = mergeVector[row_n];
                                ImGui::PushID(item->ID);
                                ImGui::TableNextRow();
                                ImGui::TableNextColumn();
                                ImGui::Text("%s", item->name);
                                ImGui::TableNextColumn();
                                ImGui::Text("%i", item->population);
                                ImGui::TableNextColumn();
                                ImGui::Text("%i", item->size);
                                ImGui::TableNextColumn();
                                ImGui::Text("%f", item->density);
                                ImGui::TableNextColumn();
                                ImGui::Text("%i", item->avgHousingCost);
                                ImGui::TableNextColumn();
                                ImGui::Text("%i", item->avgIncome);
                                ImGui::TableNextColumn();
                                ImGui::Text("%i", item->weightScore);
                                ImGui::PopID();
                            }
                        ImGui::EndTable();
                    }
                ...
                else if (e == 2)
                {
                    ...
                    if (ImGui::BeginTable("table_sorting", 7, tableFlags, ImVec2(0.0f, TEXT_BASE_HEIGHT * 100), 0.0f))
                    {
                        ...
                        if (ImGuiTableSortSpecs* sorts_specs = ImGui::TableGetSortSpecs())
                            ...
                            {
                                MyItem::s_current_sort_specs = sorts_specs; // Store in variable accessible by the sort function.
                                if (radixVector.size() > 1)
                                    qsort(&radixVector[0], radixVector.size(), sizeof(radixVector[0]), MyItem::CompareWithSortSpecs);
                                MyItem::s_current_sort_specs = NULL;
                                sorts_specs->SpecsDirty = false;
                            }
                        ImGuiListClipper clipper;
                        clipper.Begin(radixVector.size());
                        while (clipper.Step())
                            for (int row_n = clipper.DisplayStart; row_n < clipper.DisplayEnd; row_n++)
                            {
                                // Display a data item
                                City* item = radixVector[row_n];
                                ImGui::PushID(item->ID);
                                ImGui::TableNextRow();
                                ImGui::TableNextColumn();
                                ImGui::Text("%s", item->name);
                ...
                */
                ...
                ImGui::Text("");
                if (e == 1)
                    ImGui::Text("Merge sort took %i milliseconds to sort.", mergeTime);
                if (e == 2)
