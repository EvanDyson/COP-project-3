...
void Sort::mergeSort(City* arr[], int left, int right)
{
    if (left == right)
        return;
...
void Sort::merge(City* arr[], int left, int right)
{
...
void Sort::mergeSort(vector<City*> arr, int left, int right)
{
    if (left == right)
        return;
    //divides array until only one element is left
...
void Sort::merge(vector<City*> arr, int left, int right)
{
...
void Sort::radixSort(City* arr[], int size)
{
...
void Sort::radixSort(vector<City*> arr) {
    int max = arr.at(0)->weightScore;
    for (int i = 1; i < arr.size(); i++) {
        if (arr.at(i)->weightScore > max)
            max = arr.at(i)->weightScore;
    } //finds whatever weight has the most digits
    ...
    for (int i = 1; max / i > 0; i *= 10) //change i according to how many digits we want
    {
        ...
        for (int j = 0; j < arr.size(); j++) {
            count.at((arr.at(j)->weightScore / i) % 10)++;
        } //tracks how often specific digit appears
        count.at(0)--;
        ...
        for (int j = arr.size() - 1; j >= 0; j--) {
            output.at(count.at((arr.at(j)->weightScore / i) % 10)) = arr.at(j);
            count.at((arr.at(j)->weightScore / i) % 10)--;
        } //sorts values based on frequencies of digits
